...
照片、邮箱、智慧团建、截止时间、文件命名和其他提交要求只作为核对信息处理，默认不写入活动案例正文、心得正文、PPT 正文和总结表备注栏。本 Skill 也不负责照片生成、照片处理、系统录入或材料上传。总结表的**备注栏固定留空**，不填写任何内容。

## 默认篇幅与输出形式

- 活动案例：未指定时按约 1800—2800 字生成，沿用模板章节：活动策划、组织实施、活动总结、案例点评；
- 总结表：继续使用现有 Word 模板，不重新设计表格；
- 心得体会或读后感：未指定时按约 800—1200 字、默认 2 份，以自然的第一人称撰写；
- PPT：调用 ppt-master 引擎直接生成 `.pptx`，不产出 PPT 提示词或页面大纲；未指定页数时默认 10—12 页。主讲人是团支书，全篇以第一人称讲活动要传达的精神，逐页回答"这一页让听众记住什么精神"；主题史实与故事联网核实后使用，禁止把通知条款搬进页面。每页带第一人称演讲备注。
...
2. 文件统一保存到 **`E:\cc项目\团日活动材料\`**（本 Skill 根目录同级），全部产出含模板副本、内部中间目录和 PPT 生成项目目录。两条红线：不得落在 **Skill 目录内部**（会污染包体），不得落在 **C 盘**（含系统临时目录）。用户指定目录时以用户目录为准，同样受这两条约束。按 `{命名前缀}-{材料类型}-{YYYYMMDD}` 命名，例如 `软件2450-活动案例-20260618.docx`、`软件2450-心得体会-1-20260618.docx`；命名前缀（支部简称，形如 `软件2450`）在开工时询问一次，预览版加 `-预览` 后缀；
...
└─ references/
   ├─ generation-spec.md            # 各类材料的生成细则（含 PPT 内容契约与引擎不可用时的处理）
   ├─ output-format.md              # 事实核对卡、文件命名、交付报告与交付报告模板
